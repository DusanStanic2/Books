...
LAST_UPDATED = datetime.datetime.today().strftime("%B %d, %Y")
# Load data
try:
    ...
    <header>
        ...
        <div class="count-badge">
            <span id="visible-count">{total_books}</span> / <span id="total-count">{total_books}</span> Books
                ...
                <span class="update-ts">Created: {CREATED}</span>
                <span class="update-ts">Last updated: {LAST_UPDATED}</span>
    </header>
...
<script>
     // Pre-cache row text once on load — avoids DOM reads on every keystroke
    const rows = Array.from(document.querySelectorAll("#books tbody tr"));
    const rowCache = rows.map(row => row.innerText.toLowerCase());
    ...
    document.getElementById("search").addEventListener("input", function() {{
        ...
        rows.forEach((row, i) => {{
            const match = !filter || rowCache[i].includes(filter);
    ...
    function sortTable(n) {{
        ...
        const sortRows = Array.from(tbody.rows);
        ...
        // Rebuild cache to match new row order after sort
        rows.length = 0;
        rowCache.length = 0;
        Array.from(tbody.rows).forEach(row => {{
            rows.push(row);
            rowCache.push(row.innerText.toLowerCase());
        }});
    }}
</script>
